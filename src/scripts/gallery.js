...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndDisplayImages(page = 1) {
        try {
            const response = await fetch(`http://10.101.0.126:3000/api/galleryImages?page=<span class="math-inline">\{page\}&limit\=</span>${limit}`);
            if (!response.ok) {
            ...
            if (data.images && data.images.length > 0) {
                data.images.forEach(image => {
                    ...
                    imgElement.src = `http://10.101.0.126:3000${image.gallery_path}`;
